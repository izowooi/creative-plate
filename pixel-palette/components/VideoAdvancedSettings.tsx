...
import { AdvancedParam } from '@/lib/models'
import { VIDEO_MODELS, VideoModelConfig } from '@/lib/videoModels'
...
export default function VideoAdvancedSettings({ modelIds, params, onChange }: Props) {
  const [open, setOpen] = useState(false)

  const models = modelIds
    .map(id => VIDEO_MODELS.find(m => m.id === id))
    .filter((m): m is VideoModelConfig => !!m)
